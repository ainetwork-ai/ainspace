...
Generate your response now:`;
        ...
        // Fallback to original responses if Claude fails
        const fallbacks = {
        ...
        return (
            fallbacks[agentData.name as keyof typeof fallbacks] ||
            `Agent ${agentData.name} received your message from position (${agentData.position.x}, ${agentData.position.y}).`
        );
    }
}
...
Generate your commentary now:`;
        ...
        console.error('Error generating Claude commentary:', error);
        // Fallback commentary if Claude fails
        const fallbacks = [
