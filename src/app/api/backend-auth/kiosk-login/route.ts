 ...
 *
 * Returns 404 when the kiosk key is not configured: on the public web project
 * (no key) this route is effectively disabled, so even if NEXT_PUBLIC_KIOSK_MODE
 * leaked there, no kiosk login can occur.
 */
export async function POST() {
    ...
    try {
        ...
        console.error('backend-auth kiosk-login failed:', error instanceof Error ? error.message : error);
        return NextResponse.json({ error: 'kiosk login failed' }, { status: 502 });
    }
}
