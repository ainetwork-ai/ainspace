...
export default function ImportedAgentList({ agents, onSpawnAgent, onRemoveAgent }: ImportedAgentListProps) {
  return (
    <div className="flex flex-col gap-4 px-5 bg-white">
        ...
        {agents.length === 0 ? (
          <NoAgentNotice/>
            ...
            agents.map((agent) => (
                <ImportedAgentCard key={agent.url} agent={agent} onSpawnAgent={onSpawnAgent} onRemoveAgent={onRemoveAgent} />
            ))
        )}
    </div>
  )
