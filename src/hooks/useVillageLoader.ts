...
export function useVillageLoader(initialVillageSlug: string | null) {
  ...
  useEffect(() => {
    ...
    async function init() {
      setLoading(true);
      if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('village-init-start');

      try {
        // 초기 마을 메타데이터 fetch
        if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('village-metadata-fetch-start');
        ...
        if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('village-metadata-fetch-end');

        // 현재 마을의 gridIndex를 먼저 등록 (이동 가능하도록)
        updateGridIndex([metadata]);

        // 현재 마을 우선 로드 (collision 데이터 확보)
        if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('village-map-load-start');
        const loaded = await loadVillage(metadata);
        if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('village-map-load-end');
        ...
        setTimeout(() => {
          if (process.env.NEXT_PUBLIC_ENABLE_PERF_MARKS === 'true') performance.mark('nearby-villages-start');
